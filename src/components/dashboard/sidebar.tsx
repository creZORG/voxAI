...
const SidebarItem = ({ icon, label, active, onClick }: { icon: React.ReactNode, label: string, active?: boolean, onClick?: () => void }) => (
  <div 
    ...
    className={cn(
      ...
      active ? 'bg-violet-600/10 text-violet-300' : 'text-slate-400 hover:text-white hover:bg-white/5'
    )}
  >
...
export default function Sidebar({ activeTab, setActiveTab }: SidebarProps) {
  return (
    <aside className="w-64 bg-slate-900 border-r border-white/5 hidden md:flex flex-col justify-between">
      <div className="p-4 space-y-2">
        ...
        <div className="pt-4 pb-2">
          <div className="text-xs font-semibold text-slate-500 uppercase tracking-wider px-3">Analytics</div>
        </div>
