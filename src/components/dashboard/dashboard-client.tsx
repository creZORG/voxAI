...
export default function DashboardClient() {
  ...
  return (
    <div className="flex h-[calc(100vh-80px)]">
      ...
      <div className="flex-1 overflow-y-auto bg-slate-950/70 p-4 sm:p-8 custom-scrollbar">
        <header className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-2xl font-bold text-white">My Agents</h1>
            <p className="text-slate-400 text-sm">Manage your autonomous workforce.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          />
          <div className="border border-dashed border-slate-700 rounded-xl flex flex-col items-center justify-center text-slate-500 h-64 hover:bg-slate-900/50 transition cursor-pointer">
          ...
          <Table>
            ...
            <TableBody>
              {recentActivity.map((activity, index) => (
                <TableRow key={index}>
                  <TableCell className="font-medium text-white">{activity.agent}</TableCell>
                  <TableCell>{activity.action}</TableCell>
                  <TableCell className={
                    activity.outcome === 'Success' ? 'text-emerald-400' :
                    activity.outcome === 'Handoff' ? 'text-amber-400' : 'text-rose-400'
                  }>{activity.outcome}</TableCell>
                  <TableCell className="text-right">{activity.duration}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
